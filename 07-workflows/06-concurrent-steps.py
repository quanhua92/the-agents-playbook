...
class SlowStep(BaseStep):
    ...
    def __init__(self, step_id: str, deps: list[str] | None = None, duration: float = 0.1):
        self._id = step_id
        self._deps = deps or []
...
async def main():
    ...
    print(f"  (Each step waits for previous)")
    ...
    print(f"  Batches: {batches}")
    print(f"  (research_a and research_b run in parallel)")

    start = time.monotonic()
    async for event in par_wf.run("test"):
        pass
    elapsed = time.monotonic() - start
    print(f"  Total time: {elapsed:.2f}s (expected ~0.2s — saved 0.1s from concurrency)")
    print()

    # --- Full parallel (all independent) ---

    print("=== Fully Parallel DAG ===")
    full_wf = Workflow(steps=[
        SlowStep("task_1"),
        SlowStep("task_2"),
        SlowStep("task_3"),
    ])
