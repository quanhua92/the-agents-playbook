...
class OpenAIEmbeddingProvider(EmbeddingProvider):
    ...
    local models via LiteLLM, etc.) by configuring embedding_base_url.
    ...
    async def embed_batch(self, texts: list[str]) -> list[np.ndarray]:
        ...
        More efficient than calling embed() in a loop — uses the
        input array form of the embeddings endpoint.
        """
        if not texts:
            return []
